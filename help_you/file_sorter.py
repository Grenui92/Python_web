...
class FileSorter:
    ...
    def __file_checking(self, path) -> (list, list, dict):
        ...
        for item in p.iterdir():
            if item.is_dir() and item.name not in self.__absolute_folders:
                thread_checking = Thread(target=self.__file_checking, args=(os.path.join(p, item.name),))
                self.__threads.append(thread_checking)
    ...
    def __new_absolute_folders_create() -> dict:
        absolute_folder = {'archives': ['zip', 'gz', 'tar'],
                           'video': ['avi', 'mp4', 'mov', 'mkv'],
                           'audio': ['mp3', 'ogg', 'wav', 'amr'],
                           'documents': ['doc', 'docx', 'txt', 'pdf', 'xlsx', 'pptx'],
                           'images': ['jpeg', 'png', 'jpg', 'svg'], 'others': [],
